...
$setSessionHandler = session_set_save_handler("on_session_start",   "on_session_end",
			"on_session_read",    "on_session_write",
			"on_session_destroy", "on_session_gc");
	
if(!$setSessionHandler)
{
	error_log("[ERROR]" . "-failed to set session handler for  ". session_id());
}
...
include_once('uon_specific/classes/clsAuthenticate.php'); 
 ...
 {
 	 	if(isset($_SESSION)||isset($_COOKIE["_lgd"]))
 	 	{
 	 		$_SESSION = array();
			if (isset($_COOKIE[session_name()])) setcookie(session_name(), '', time()-42000, '/');
			if (isset($_COOKIE["_uonA"])) setcookie("_uonA", '', time()-42000, '/');
			if (isset($_COOKIE["_lgd"])) setcookie("_lgd", '', time()-42000, '/');
			session_destroy();
 	 	}	
